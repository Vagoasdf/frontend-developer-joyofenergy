import { getReadings, groupByDay, sortByTime } from './data/reading'
...
describe('#reading', function () {
  ...
  describe('#groupedByDay', () => {
    ...
    it('should get readings grouped by day', async () => {
      ...
      expect(
        groupedReadings.find((reading) => reading.time === new Date(2021, 12, 17).getTime()).value
      ).toBe(90)
      expect(
        groupedReadings.find((reading) => reading.time === new Date(2021, 12, 16).getTime()).value
      ).toBe(35)
    })
  })
